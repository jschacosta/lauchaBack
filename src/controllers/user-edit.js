...
export const editPass = async (req, res, next) => {
  try {
    const _id = req.params.id;
    const body = req.body;
    const DB = await User.findById({ _id: _id });
...
export const editTorneo = async (req, res, next) => {
  try {
    const body = req.body;
    const usuarioDB = await User.findOne({ _id: body.usuarioId });
    usuarioDB.torneos.push(body.torneoId);
    const nuevoUser = await User.findByIdAndUpdate(body.usuarioId, usuarioDB, {
      new: true,
    });
...
export const deleteUserTorneo = async (req, res, next) => {
  const _id = req.params.id;
  try {
    const torneoDB = await torneo.find();
    const usuarioDB = await User.findOne({ _id: _id });
    const idTorneo = torneoDB[0]._id;
    const indexTorneo = usuarioDB.torneos.findIndex(
      (item) => item === idTorneo
    );
    usuarioDB.torneos.splice(indexTorneo, 1);

    const indexJugador = torneoDB[0].players.findIndex(
      (item) => item._id === _id
    );
    torneoDB[0].players.splice(indexJugador, 1);
    const nuevoUser = await User.findByIdAndUpdate(_id, usuarioDB, {
      new: true,
    });
    const nuevoTorneo = await torneo.findByIdAndUpdate(idTorneo, torneoDB[0], {
      new: true,
    });
